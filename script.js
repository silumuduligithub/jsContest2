var information = [{"id":1,"first_name":"Chadwick","last_name":"Ayre","email":"cayre0@cam.ac.uk","gender":"Male","img_src":"https://robohash.org/corporisquiaperiam.png?size=50x50&set=set1","class":11,"marks":18,"passing":false,"city":"Moorreesburg"},
...
// data in asending order based on name

function search(){
    let name = document. querySelector('input'). value;
    console.log(name);
    for(let i = 0; i < information.length; i++){
        if(information[i].first_name == name || information[i].last_name == name || information[i].email == name){
            window.alert(information[i].first_name +" "+information[i].last_name + " "+ information[i].email +" "+ information[i].class +" "+ information[i].gender);
            return;
        }
    }
    window.alert("student not found");
}
function printDataAsending(){
    let informations = information.sort((a, b)=>{return  a.first_name.localeCompare(b.first_name)});
    let body = document.getElementsByTagName("BODY")[0];
    let parentDiv = document.getElementsByTagName("SCRIPT")[0];
    let childDiv = document.createElement("div");
    body.insertBefore(childDiv, parentDiv);
    childDiv.style.display = 'flex';
    childDiv.style.textAlign = 'center';
    childDiv.style.alignContent = 'center';
    childDiv.style.width = '100%'
    childDiv.justifyContent = 'center';
    childDiv.style.marginTop = '40px';
    childDiv.style.marginLeft = '25vw';
    ...
    for(let i = 0; i < informations.length; i++){
        ...
        for(let j = 0; j < 7; j++){
            ...
            if(j == 0){
                td.appendChild(document.createTextNode(informations[i].id));
            }
            else if(j == 1){
                td.appendChild( document.createTextNode(informations[i].first_name +" "+informations[i].last_name));
            }
            else if(j == 2){
                td.appendChild(document.createTextNode(informations[i].gender));
            }
            else if(j == 3){
                td.appendChild(document.createTextNode(informations[i].class));
            }
            else if(j == 4){
                td.appendChild(document.createTextNode(informations[i].marks));
            }
            else if(j == 5){
                if(informations[i].passing == true)
                    td.appendChild(document.createTextNode("YES"));
                else
                    td.appendChild(document.createTextNode("NO"));
            }
            else if(j == 6){
                td.appendChild(document.createTextNode(informations[i].email));
            }
            tr.appendChild(td);
        }
    }
    console.log(informations[0].length);
    childDiv.appendChild(table);
}

// data in desending order based on name
function printDataDesending(){
    let informations = information.sort((a, b)=>{return  b.first_name.localeCompare(a.first_name)});
    let div1 = document.getElementsByClassName("sess")[0];
    let div2 = document.createElement("div");
    div1.appendChild(div2);
    div1.style.alignItems = 'center';
    // div2.style.marginTop = '2700px'
    div2.style.display = 'block';
    div2.style.justifyContent = 'center';
    div2.style.width = '80%';
    div2.style.marginTop = '50px';
    div2.style.marginLeft = '25vw';
    let table = document.createElement("TABLE");
    table.border = 1;
    let tableBody = document.createElement("tbody");
    table.appendChild(tableBody);
...
function printDataInMarks(){
    let informations = information.sort((a, b)=>{return  a.marks - b.marks});
    let div1 = document.getElementsByClassName("sess")[0];
    let div2 = document.createElement("div");
    div1.appendChild(div2);
    div1.style.alignItems = 'center';
...
// sorth based on the pass mark

function printDataInPassing(){
    let informations = information.sort((a, b)=>{return  b.passing - a.passing});
    let div1 = document.getElementsByClassName("sess")[0];
    let div2 = document.createElement("div");
    div1.appendChild(div2);
    div1.style.alignItems = 'center';
...
// sort based on class
function printDataInClass(){
    let informations = information.sort((a, b)=>{return  a.class - b.class});
    let div1 = document.getElementsByClassName("sess")[0];
    let div2 = document.createElement("div");
    div1.appendChild(div2);
    div1.style.alignItems = 'center';
...
function printDataInGender(){
    ...
    MaleTable.border = 1;
    let tableBody = document.createElement("tbody");
    MaleTable.appendChild(tableBody);
   
    for(let i = 0; i < information.length; i++){
        let tr = document.createElement("tr");
        tableBody.appendChild(tr);
        for(let j = 0; j < 7; j++){
            if(information[i].gender != "Male")continue;
            let td = document.createElement("td");
            td.style.width = '100px';
            td.style.textAlign = 'center';
            td.style.alignItems = 'center';
            if(j == 0 && information[i].gender == "Male"){
                td.appendChild(document.createTextNode(information[i].id));
            }
            else if(j == 1 && information[i].gender == "Male"){
                td.appendChild( document.createTextNode(information[i].first_name +" "+information[i].last_name));
            }
            else if(j == 2 && information[i].gender == "Male"){
                td.appendChild(document.createTextNode(information[i].gender));
            }
            else if(j == 3 && information[i].gender == "Male"){
                td.appendChild(document.createTextNode(information[i].class));
            }
            else if(j == 4 && information[i].gender == "Male"){
                td.appendChild(document.createTextNode(information[i].marks));
            }
            else if(j == 5 && information[i].gender == "Male"){
                if(information[i].passing == true)
                    td.appendChild(document.createTextNode("YES"));
                else
                    td.appendChild(document.createTextNode("NO"));
            }
            else if(j == 6 && information[i].gender == "Male"){
                td.appendChild(document.createTextNode(information[i].email));
            }
            tr.appendChild(td);
        }
    }
    console.log(information[0].length);
    div2.appendChild(MaleTable);
    let br = document.createElement("br");
    div1.append(br);

    let femaleTable = document.createElement("TABLE");
    femaleTable.border = 1;
    let femaleTableBody = document.createElement("tbody");
    femaleTable.appendChild(femaleTableBody);
   
    for(let i = 0; i < information.length; i++){
        let tr = document.createElement("tr");
        femaleTableBody.appendChild(tr);
        for(let j = 0; j < 7; j++){
            if(information[i].gender != "Female")continue;
            let td = document.createElement("td");
            td.style.width = '100px';
            td.style.textAlign = 'center';
            td.style.alignItems = 'center';
            if(j == 0 && information[i].gender == "Female"){
                td.appendChild(document.createTextNode(information[i].id));
            }
            else if(j == 1 && information[i].gender == "Female"){
                td.appendChild( document.createTextNode(information[i].first_name +" "+information[i].last_name));
            }
            else if(j == 2 && information[i].gender == "Female"){
                td.appendChild(document.createTextNode(information[i].gender));
            }
            else if(j == 3 && information[i].gender == "Female"){
                td.appendChild(document.createTextNode(information[i].class));
            }
            else if(j == 4 && information[i].gender == "Female"){
                td.appendChild(document.createTextNode(information[i].marks));
            }
            else if(j == 5 && information[i].gender == "Female"){
                if(information[i].passing == true)
                    td.appendChild(document.createTextNode("YES"));
                else
                    td.appendChild(document.createTextNode("NO"));
            }
            else if(j == 6 && information[i].gender == "Female"){
                td.appendChild(document.createTextNode(information[i].email));
            }
            tr.appendChild(td);
        }
